...
async def load_mcp_tools(user_context: UserContext) -> McpToolsResult:
    ...
    try:
        ...
        if missing:
            logger.warning(
                "holiday-request-agent: missing expected MCP tools: %s",
                ", ".join(missing),
            )

        logger.info(
            "holiday-request-agent MCP tools loaded: %s",
            ", ".join(tool_map.keys()),
        )
        ...
        logger.warning(
            "holiday-request-agent: failed to load MCP tools — agent will run without them: %s",
            err,
        )
        return McpToolsResult(tools=_make_empty_tools(), cleanup=None)
